...
global username_entry, passwd_entry, email_entry, alpha_var, omega_var, nickname_entry
h_label = {}
h_entry = {}
h_button = {}
...
def submit_add_entry():
    add_entry(username_entry, passwd_entry, email_entry, alpha_var, omega_var, nickname_entry, default_passwd_var)
...
h_label[0] = label(frame, "Enter your nickname", 1, 0)
h_entry[0] = nickname_entry = entry(frame, 1, 1, 2)

label(frame, "Enter your username:", 2, 0)
username_entry = entry(frame, 2, 1, 2)

label(frame, "Enter your password:", 3, 0)
passwd_entry = entry(frame, 3, 1, 2)
passwd_entry.config(state=tk.DISABLED)

h_label[1] = label(frame, "Enter your Email:", 5, 0)
h_entry[1] = email_entry = entry(frame, 5, 1, 2)

default_passwd_var = tk.IntVar(value=1)
check_button(frame, "Use default password", default_passwd_var, 1, 0, lambda: passwd_toggle_entry(passwd_entry, default_passwd_var), 4, 0, 4, sticky="we")

create_button(frame, "Create File", submit_create_file, 6, 1, bg="#4CAF50", fg="black")
h_button[0] = create_button(frame, "Excel", submit_add_entry, 7, 2, bg="#FF9800", fg="white")
create_button(frame, "Sign up", open_website, 6, 2, bg="#FF5100", fg='#000000')
